...
const Constants = require('./util/codes');
...
converterCommand.pickCommand = function pickCommand() {
    ...
    for (let command of ['./ffmpeg', './avconv', 'ffmpeg', 'avconv']) {
        let res = ChildProcess.spawnSync(command, ['-encoders']);
        if (!res.error) {
                ...
                case OPCodes.HELLO: {
                    ...
                    DNS.lookup(this.endpoint, (err, address) => { // RIP DNS
                        ...
                        this.udpSocket.once('message', (packet) => {
                            ...
                            let localPort = parseInt(packet.readUIntLE(packet.length - 2, 2).toString(10));
    ...
    */
    ...
    _createPacket(_buffer) {
        ...
        if (!NaCl) {
            ...
            let buffer = NaCl.secretbox(_buffer, this.nonce, this.secret);
    ...
    registerReceiveEventHandler() {
        this.udpSocket.on('message', (msg) => {
            ...
            if (data[0] === 0xBE && data[1] === 0xDE) { // RFC5285 Section 4.2: One-Byte Header
                let rtpHeaderExtensionLength = data[2] << 8 | data[3];
                let index = 4;
                let byte;
