...
class DCAOpusTransformer extends BaseTransformer {
    ...
    process(buffer) {
        ...
        let opusLen = buffer.readInt16LE(buffer._index);
        buffer._index += 2;
    ...
    _transform(chunk, enc, cb) {
        ...
        if (!this.head) {
            if (chunk.length < 4) {
                ...
                let dcaVersion = chunk.slice(0, 4);
                if (dcaVersion[0] !== 68 || dcaVersion[1] !== 67 || dcaVersion[2] !== 65) { // DCA0 or invalid
                    ...
                    let jsonLength = chunk.slice(4, 8).readInt32LE(0);
                    if (chunk.length < 8 + jsonLength) {
                        this._remainder = chunk;
                        return cb();
                    }
                    let jsonMetadata = chunk.slice(8, 8 + jsonLength);
