...
class WebmOpusTransformer extends BaseTransformer {
    ...
    process(type, info) {
        if (type === TAG_TYPE_TAG) {
            ...
            if (info.name === 'CodecPrivate') {
                let head = info.data.toString('utf8', 0, 8);
                if (head !== 'OpusHead') {
    ...
    readTag(buffer) {
        let tagSize = this.getVIntLength(buffer, buffer._index);
        if (tagSize === null) {
            return false;
        }

        let size = this.getVIntLength(buffer, buffer._index + tagSize);
        if (size === null) {
            return false;
        }

        let tagStr = buffer.toString('hex', buffer._index, buffer._index + tagSize);
